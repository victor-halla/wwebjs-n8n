        ...
        (0, tournament_1.astVisit)(ast, {
            visitVariableDeclarator(path) {
                ...
                if (node.id.type === 'Identifier' && RESERVED_VARIABLE_NAMES.has(node.id.name)) {
                    throw new errors_1.ExpressionReservedVariableError(node.id.name);
                }
            },
            visitFunction(path) {
                this.traverse(path);
                ...
                for (const param of node.params) {
                    if (param.type === 'Identifier' && RESERVED_VARIABLE_NAMES.has(param.name)) {
                        throw new errors_1.ExpressionReservedVariableError(param.name);
                    }
                }
            },
            visitCatchClause(path) {
                ...
                if (node.param?.type === 'Identifier' && RESERVED_VARIABLE_NAMES.has(node.param.name)) {
                    throw new errors_1.ExpressionReservedVariableError(node.param.name);
                }
            },
