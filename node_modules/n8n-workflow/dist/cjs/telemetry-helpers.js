(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "@n8n/errors", "./constants", "./evaluation-helpers", "./interfaces", "./node-helpers", "./utils"], factory);
    }
    ...
    exports.getNodeRole = getNodeRole;
    ...
    function generateNodesGraph(workflow, nodeTypes, options) {
        ...
        otherNodes.forEach((node, index) => {
            ...
            else if (node.type === constants_1.GUARDRAILS_NODE_TYPE) {
                nodeItem.operation = node.parameters.operation;
                const usedGuardrails = Object.keys(node.parameters?.guardrails ?? {});
                nodeItem.used_guardrails = usedGuardrails;
            }
            else if (node.type === constants_1.OPENAI_CHAT_LANGCHAIN_NODE_TYPE) {
            ...
            else if (node.type === constants_1.MCP_CLIENT_TOOL_NODE_TYPE || node.type === constants_1.MCP_CLIENT_NODE_TYPE) {
                nodeItem.mcp_client_auth_method = (node.parameters?.authentication ?? 'none');
            }
            else {
                try {
                    const nodeType = nodeTypes.getByNameAndVersion(node.type, node.typeVersion);
     ...
     */
    function getNodeRole(nodeName, connections, nodeTypes, nodes) {
        ...
        const nodeTypeDescription = node
            ...
            : null;
        const isSubnode = (0, node_helpers_1.isSubNodeType)(nodeTypeDescription);
    ...
    function hasOutgoing(nodeName, connections) {
        const mainConnections = connections[nodeName]?.[interfaces_1.NodeConnectionTypes.Main];
        if (!mainConnections) {
            return false;
        }
    ...
    function hasIncomingMain(nodeName, connections) {
        ...
        for (const sourceNode of Object.keys(connections)) {
            ...
            const mainConnections = sourceConnections?.[interfaces_1.NodeConnectionTypes.Main];
            if (!mainConnections)
                continue;
            for (const outputIndex of mainConnections) {
                if (!outputIndex)
